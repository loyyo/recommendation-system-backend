...
class RecommendationService:
    ...
    def _validate_data(self):
        ...
        if not self.user_data["product_id"].isin(self.product_data["product_id"]).all():
            raise ValueError("Some product IDs in user_data are missing from product_data.")
    ...
    def _generate_cb_recommendations(self, user_id):
        ...
        if recommendations:
            ...
            filtered_recommendations = recommendations[~recommendations["product_id"].isin(interacted_products)].copy()

            if filtered_recommendations.empty:
                filtered_recommendations = recommendations.head(5).copy()

            if not filtered_recommendations.empty:
                filtered_recommendations.loc[:, "cb_score"] = np.linspace(1, 0.5, len(filtered_recommendations))
    ...
    def _merge_recommendations(self, cb_recs, cf_recs):
        ...
        # Ensure proper indices for merging
        if not cb_recs.empty:
            cb_recs = cb_recs.set_index("product_id", drop=False)
        else:
            cb_recs = pd.DataFrame(columns=["product_id", "product_description", "cb_score"])

        if not cf_recs.empty:
            cf_recs = cf_recs.set_index("product_id", drop=False)
        else:
            cf_recs = pd.DataFrame(columns=["product_id", "product_description", "cf_score"])

        # Perform the join
        merged = cb_recs.join(cf_recs, how="outer", lsuffix="_cb", rsuffix="_cf").fillna(0)

        # Assign merged product_id and description with proper fallback
        merged["product_id"] = merged.index
        if "product_description_cb" in merged.columns and "product_description_cf" in merged.columns:
            merged["product_description"] = merged["product_description_cf"].combine_first(
                merged["product_description_cb"])
        elif "product_description_cb" in merged.columns:
            merged["product_description"] = merged["product_description_cb"]
        elif "product_description_cf" in merged.columns:
            merged["product_description"] = merged["product_description_cf"]
        else:
            merged["product_description"] = None
    ...
    def _get_similar_products(self, product_id, top_n=5):
        if product_id not in self.product_data["product_id"].values:
            print(f"Debug: Product ID {product_id} not found in product_data.")
